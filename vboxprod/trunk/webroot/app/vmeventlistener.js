	...
	_getEvents : function(){
		...
		return vboxEventListener._requestQueue.addReq(function(){
			...
			return $.when(new Date().getTime(), vboxAjaxRequest('getEvents',{'_persist':vboxEventListener._persist})).done(function(lastTime,d) {
				...
				if(!d.key || (d.key != Ext.get('vboxPane').data('vboxConfig').key)) return;
				...
				if(d.responseData && d.responseData.length) {
					...
					for(var i = 0; i < d.responseData.length; i++) {
						...
						doc.fireEvent('vbox' + d.responseData[i].eventType, [d.responseData[i]]);
						
					}
					
					// Trigger event list queue
					doc.fireEvent('vboxEvents', [d.responseData]);
